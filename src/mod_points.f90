...
! -----------------------------------------------------------
    ...
    subroutine from_IJK_to_XYZ()
        ...
        call DMDAVecGetArray(meshDA, QQ_X, tmp, ierr)
        tmp(:,is:ie,js:je,ks:ke) = qqx(:,is:ie,js:je,ks:ke)
        call DMDAVecRestoreArray(meshDA, QQ_X, tmp, ierr)
        call DMGlobalToLocalBegin(meshDA, QQ_X, INSERT_VALUES, QQX_local, ierr)
        call DMDAVecGetArray(meshDA, QQ_Y, tmp, ierr)
        tmp(:,is:ie,js:je,ks:ke) = qqy(:,is:ie,js:je,ks:ke)
        call DMDAVecRestoreArray(meshDA, QQ_Y, tmp, ierr)
        ...
        call DMDAVecGetArray(meshDA, QQ_Z, tmp, ierr)
        tmp(:,is:ie,js:je,ks:ke) = qqz(:,is:ie,js:je,ks:ke)
        call DMDAVecRestoreArray(meshDA, QQ_Z, tmp, ierr)
        ...
        call DMDAVecGetArrayRead(meshDA, QQX_local, tmp, ierr)
        qq_x_local_array(:,igs:ige,jgs:jge,kgs:kge)=tmp(:,igs:ige,jgs:jge,kgs:kge)
        call DMDAVecRestoreArrayRead(meshDA, QQX_local, tmp, ierr)
        call DMDAVecGetArrayRead(meshDA, QQY_local, tmp, ierr)
        qq_y_local_array(:,igs:ige,jgs:jge,kgs:kge)=tmp(:,igs:ige,jgs:jge,kgs:kge)
        call DMDAVecRestoreArrayRead(meshDA, QQY_local, tmp, ierr)
        call DMDAVecGetArrayRead(meshDA, QQZ_local, tmp, ierr)
        qq_z_local_array(:,igs:ige,jgs:jge,kgs:kge)=tmp(:,igs:ige,jgs:jge,kgs:kge)
        call DMDAVecRestoreArrayRead(meshDA, QQZ_local, tmp, ierr)
        select case (lns_mode)
        case(2)
    ...
    subroutine delivery_by_dmda()
        ...
        call DMDAVecGetArray(meshDA, QQ_, tmp, ierr)
        tmp(:,is:ie,js:je,ks:ke) = qq(:,is:ie,js:je,ks:ke)
        call DMDAVecRestoreArray(meshDA, QQ_, tmp, ierr)
        call DMGlobalToLocalBegin(meshDA, QQ_, INSERT_VALUES, QQ_local, ierr)

        call DMDAVecGetArray(meshDA, QQ_XX, tmp, ierr)
        tmp(:,is:ie,js:je,ks:ke) = qqxx(:,is:ie,js:je,ks:ke)
        call DMDAVecRestoreArray(meshDA, QQ_XX, tmp, ierr)
        ...
        call DMDAVecGetArray(meshDA, QQ_YY, tmp, ierr)
        tmp(:,is:ie,js:je,ks:ke) = qqyy(:,is:ie,js:je,ks:ke)
        call DMDAVecRestoreArray(meshDA, QQ_YY, tmp, ierr)
        ...
        call DMDAVecGetArray(meshDA, QQ_ZZ, tmp, ierr)
        tmp(:,is:ie,js:je,ks:ke) = qqzz(:,is:ie,js:je,ks:ke)
        call DMDAVecRestoreArray(meshDA, QQ_ZZ, tmp, ierr)
        ...
        call DMDAVecGetArray(meshDA, QQ_XY, tmp, ierr)
        tmp(:,is:ie,js:je,ks:ke) = qqxy(:,is:ie,js:je,ks:ke)
        call DMDAVecRestoreArray(meshDA, QQ_XY, tmp, ierr)
        ...
        call DMDAVecGetArray(meshDA, QQ_XZ, tmp, ierr)
        tmp(:,is:ie,js:je,ks:ke) = qqxz(:,is:ie,js:je,ks:ke)
        call DMDAVecRestoreArray(meshDA, QQ_XZ, tmp, ierr)
        ...
        call DMDAVecGetArray(meshDA, QQ_YZ, tmp, ierr)
        tmp(:,is:ie,js:je,ks:ke) = qqyz(:,is:ie,js:je,ks:ke)
        call DMDAVecRestoreArray(meshDA, QQ_YZ, tmp, ierr)
        ...
        call DMDAVecGetArrayRead(meshDA, QQ_local, tmp, ierr)
        qq(:,igs:ige,jgs:jge,kgs:kge)=real(tmp(:,igs:ige,jgs:jge,kgs:kge))
        call DMDAVecRestoreArrayRead(meshDA, QQ_local, tmp, ierr)

        call DMDAVecGetArrayRead(meshDA, QQX_local, tmp, ierr)
        qqx(:,igs:ige,jgs:jge,kgs:kge)=real(tmp(:,igs:ige,jgs:jge,kgs:kge))
        call DMDAVecRestoreArrayRead(meshDA, QQX_local, tmp, ierr)

        call DMDAVecGetArrayRead(meshDA, QQY_local, tmp, ierr)
        qqy(:,igs:ige,jgs:jge,kgs:kge)=real(tmp(:,igs:ige,jgs:jge,kgs:kge))
        call DMDAVecRestoreArrayRead(meshDA, QQY_local, tmp, ierr)

        call DMDAVecGetArrayRead(meshDA, QQZ_local, tmp, ierr)
        qqz(:,igs:ige,jgs:jge,kgs:kge)=real(tmp(:,igs:ige,jgs:jge,kgs:kge))
        call DMDAVecRestoreArrayRead(meshDA, QQZ_local, tmp, ierr)

        call DMDAVecGetArrayRead(meshDA, QQXX_local, tmp, ierr)
        qqxx(:,igs:ige,jgs:jge,kgs:kge)=real(tmp(:,igs:ige,jgs:jge,kgs:kge))
        call DMDAVecRestoreArrayRead(meshDA, QQXX_local, tmp, ierr)

        call DMDAVecGetArrayRead(meshDA, QQYY_local, tmp, ierr)
        qqyy(:,igs:ige,jgs:jge,kgs:kge)=real(tmp(:,igs:ige,jgs:jge,kgs:kge))
        call DMDAVecRestoreArrayRead(meshDA, QQYY_local, tmp, ierr)

        call DMDAVecGetArrayRead(meshDA, QQZZ_local, tmp, ierr)
        qqzz(:,igs:ige,jgs:jge,kgs:kge)=real(tmp(:,igs:ige,jgs:jge,kgs:kge))
        call DMDAVecRestoreArrayRead(meshDA, QQZZ_local, tmp, ierr)

        call DMDAVecGetArrayRead(meshDA, QQXY_local, tmp, ierr)
        qqxy(:,igs:ige,jgs:jge,kgs:kge)=real(tmp(:,igs:ige,jgs:jge,kgs:kge))
        call DMDAVecRestoreArrayRead(meshDA, QQXY_local, tmp, ierr)

        call DMDAVecGetArrayRead(meshDA, QQXZ_local, tmp, ierr)
        qqxz(:,igs:ige,jgs:jge,kgs:kge)=real(tmp(:,igs:ige,jgs:jge,kgs:kge))
        call DMDAVecRestoreArrayRead(meshDA, QQXZ_local, tmp, ierr)

        call DMDAVecGetArrayRead(meshDA, QQYZ_local, tmp, ierr)
        qqyz(:,igs:ige,jgs:jge,kgs:kge)=real(tmp(:,igs:ige,jgs:jge,kgs:kge))
        call DMDAVecRestoreArrayRead(meshDA, QQYZ_local, tmp, ierr)

        call VecDestroy(QQ_,ierr);  call VecDestroy(QQ_local,ierr)
